...
export class ConfirmDangerousSettingModal extends ConfirmationModal {
    constructor(
        ...
        this.contentEl.createEl("p", {
            text: this.t(
                "This setting is stored on this device only; enable it separately on each device where you use this vault."
            ),
        });
        ...
        if (Platform.isMobile) {
            this.addButton((btn) => {
                enableBtn = btn;
                btn.setButtonText(this.t("I understand the risks, enable"))
                    .setCta()
                    .onClick(async () => {
                        await onConfirm();
                    });
            ...
            this.addCheckbox(this.t("I understand the risks"), (checked) => {
                enableBtn.setDisabled(!checked);
            }).addButton((btn) => {
                enableBtn = btn;
                btn.setButtonText(this.t("Enable"))
                    .setCta()
                    .setDisabled(true)
                    .onClick(async () => {
                        await onConfirm();
